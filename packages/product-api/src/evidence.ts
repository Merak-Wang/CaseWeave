...
export function candidateEvidence(state: RetrievalState, candidateRef: string) {
  ...
  const candidate = state.candidates.find(c => c.ref === candidateRef) ?? state.candidateHistory.find(c => c.ref === candidateRef)
  ...
  const citations = state.promotedEvidence.filter(e => e.candidateRef === candidateRef
    ...
    && (judgment?.basis === 'proxy' || state.modelVisibleEvidenceIds?.includes(e.evidenceId) || state.contextManifests?.some(m => m.inputGeneration === (state.inputGeneration ?? 0)
      && m.measurement === 'dsh_request' && m.evidenceIds.includes(e.evidenceId)))).map(e => ({
