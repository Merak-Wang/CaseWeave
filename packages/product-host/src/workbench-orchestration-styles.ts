...
.live-work { position: relative; border: 1px solid #e4e2e2; border-radius: 17px; background: linear-gradient(115deg,#f4f2f2,#fcfdfa 75%); margin: 22px 0 24px; padding: 20px 22px 15px; overflow: hidden; }
.live-work::after { content: ''; position: absolute; bottom: 0; height: 2px; left: 0; right: 0; background: linear-gradient(90deg,transparent,#bb676e,transparent); opacity: 0; }
...
.live-copy .eyebrow { font-size: 10px; letter-spacing: 1.6px; color: #7e7b7b; }
.live-copy p { margin: 2px 0 0; font-size: 14px; font-weight: 500; line-height: 1.65; }
.ai-orbit { position: relative; flex: 0 0 40px; height: 40px; display: grid; place-items: center; color: #89353c; background: #ebe9e9; border-radius: 50%; }
.ai-orbit .brand-mark { width: 23px; height: 23px; }
.ai-orbit::before { content: ''; position: absolute; inset: -3px; border-radius: 50%; border: 1px solid #d5c6c7; border-top-color: #99484f; }
...
#elapsed { color: #858282; font: 11px/1.4 ui-monospace,Consolas,monospace; white-space: nowrap; font-variant-numeric: tabular-nums; }
.stage-rail { display: flex; list-style: none; gap: 0; padding: 0; margin: 22px 0 0; }
.stage-rail li { position: relative; display: flex; align-items: center; gap: 7px; font-size: 11px; color: #999797; flex: 1; min-width: 0; }
.stage-rail li:not(:last-child)::after { content: ''; height: 1px; background: #e2e0e0; flex: 1; margin: 0 12px; min-width: 5px; }
.stage-rail li:last-child { flex: 0 1 auto; }
.stage-number { display: grid; place-items: center; width: 20px; height: 20px; border: 1px solid #dfdddd; border-radius: 50%; font-size: 10px; flex-shrink: 0; }
.stage-rail [data-state=active] { color: #7d262d; font-weight: 600; }
...
.activity-item:not(:last-child)::before { content: ''; position: absolute; left: 13px; top: 28px; bottom: 0; width: 1px; background: #e4e2e2; }
.activity-dot { display: grid; place-items: center; flex: 0 0 27px; height: 27px; border-radius: 9px; background: #efeded; color: #8f5c60; font-size: 16px; }
...
.activity-item[data-kind=user] .activity-dot { background: #eceaea; color: #757373; }
.activity-item[data-kind=delegate] .activity-dot { background: #f2f0f0; color: #957e80; }
.activity-item[data-kind=finding] .activity-dot, .activity-item[data-kind=finish] .activity-dot { background: #e9e6e6; color: #873139; }
.activity-log[data-busy=true] .activity-item:last-child .activity-dot { position: relative; }
.activity-log[data-busy=true] .activity-item:last-child .activity-dot::after { content: ''; position: absolute; inset: -4px; border: 1.5px solid #b67c8133; border-top-color: #9e4f56; border-radius: 12px; animation: orbit-turn 2.6s linear infinite; }
.stage-rail [data-state=active] .stage-number { position: relative; }
.stage-rail [data-state=active] .stage-number::after { content: ''; position: absolute; inset: -4px; border: 1px solid transparent; border-top-color: #9b454d; border-radius: 50%; animation: orbit-turn 2s linear infinite; }
.task-status[data-state=running] .status-dot { width: 12px; height: 12px; border: 2px solid #ac797d44; border-top-color: var(--accent); background: transparent; box-shadow: none; animation: orbit-turn 1.5s linear infinite; }
...
.activity-item small { color: #807e7e; font-size: 10px; }
.activity-item p { margin: 5px 0 0; font-size: 13px; line-height: 1.85; overflow-wrap: anywhere; white-space: pre-wrap; }
.orchestration-map { position: relative; border: 1px solid #e5e3e3; border-radius: 18px; padding: 25px 24px; background-color: #f4f4f4; background-image: radial-gradient(#cccaca80 .7px, transparent .7px); background-size: 12px 12px; }
.coordinator-node, .synthesis-node { position: relative; display: flex; gap: 12px; align-items: center; width: fit-content; max-width: 100%; margin: auto; border: 1px solid #dddbdb; border-radius: 12px; padding: 13px 20px; background: #fcfdfb; z-index: 1; }
.coordinator-node strong, .synthesis-node strong { font-size: 13px; font-weight: 550; }
.coordinator-node small, .synthesis-node small { display: block; color: #908e8e; font-size: 10px; margin-top: 3px; }
.node-icon, .synthesis-node>span { display: grid; place-items: center; width: 30px; height: 30px; background: #eceaea; border-radius: 9px; color: #8b5b5f; flex-shrink: 0; }
...
.expert-lanes::before, .expert-lanes::after { content: ''; position: absolute; width: 1px; height: 38px; background: #d1cece; left: 50%; top: 0; }
.expert-lanes::after { top: auto; bottom: 0; }
.expert-node { position: relative; min-width: 0; border: 1px solid #dedcdc; border-radius: 12px; background: #fff; padding: 15px; box-shadow: 0 3px 8px #4d404104; }
.expert-node::before { content: ''; position: absolute; left: 50%; top: -20px; height: 20px; width: 1px; background: #d1cece; }
.expert-node::after { content: ''; position: absolute; top: -20px; left: -8px; right: -8px; height: 1px; background: #d1cece; }
.expert-node:first-child::after { left: 50%; }
.expert-node:last-child::after { right: 50%; }
.expert-node[data-state=running] { border-color: #c2888d; box-shadow: 0 0 0 3px #aa616810; }
.expert-node[data-state=running]::before { background: linear-gradient(180deg,#d1cece,#9f5057,#d1cece); background-size: 100% 200%; animation: branch-flow 1.5s linear infinite; }
.expert-node-head { display: flex; align-items: center; gap: 8px; }
.expert-node-head strong { font-size: 12px; font-weight: 550; flex: 1; }
.expert-avatar { display: grid; place-items: center; flex: 0 0 28px; height: 28px; border-radius: 9px; background: #eae8e8; color: #7d6668; font-size: 11px; }
.expert-node:nth-child(3n + 2) .expert-avatar { color: #93545a; background: #f1e5e6; }
.expert-node:nth-child(3n) .expert-avatar { color: #aa6a6f; background: #f3f1f1; }
.expert-state { font-size: 9px; color: #85797a; white-space: nowrap; }
.expert-node[data-state=running] .expert-state { color: #8d3a42; }
.expert-goal { font-size: 12px; line-height: 1.8; margin: 14px 0 12px; color: #636161; overflow-wrap: anywhere; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
.expert-action { display: flex; gap: 6px; align-items: center; font-size: 10px; color: #8e8283; margin: 0 0 12px; }
.expert-node[data-state=running] .expert-action::before { content: ''; width: 5px; height: 5px; border-radius: 50%; background: #ae656c; animation: soft-breathe 1.5s ease-in-out infinite; }
.expert-knowledge { display: flex; flex-wrap: wrap; gap: 5px; font-size: 10px; margin-bottom: 15px; }
button.knowledge-chip { display: inline-block; min-height: 26px; border: 1px solid #e3e1e1; background: #f7f5f5; border-radius: 6px; padding: 4px 7px; color: #7c6768; font-size: 10px; text-align: left; overflow-wrap: anywhere; }
.expert-counts { display: block; color: #9d9a9a; font-size: 10px; }
...
.library-toolbar input { width: min(100%,340px); background: #f1efef; border: 1px solid transparent; border-radius: 10px; padding: 10px 14px; font-size: 12px; outline-offset: 2px; }
.domain-filters { display: flex; flex-wrap: wrap; gap: 6px; margin: 16px 0 20px; }
button.domain-filter { font-size: 11px; padding: 6px 10px; min-height: 30px; border-radius: 7px; border-color: transparent; background: transparent; color: #8f8384; }
button.domain-filter[aria-pressed=true] { color: #764449; background: #eae8e8; border-color: #e2e0e0; }
.knowledge-list { display: grid; grid-template-columns: repeat(2,minmax(0,1fr)); gap: 13px; }
button.knowledge-card { display: block; min-width: 0; text-align: left; padding: 18px 19px; border-radius: 12px; background: #fff; border-color: #e5e3e3; }
button.knowledge-card:hover:not(:disabled) { background: #fafafa; border-color: #d1c2c3; box-shadow: 0 5px 18px #6d585908; }
.knowledge-card-meta { display: flex; justify-content: space-between; gap: 8px; color: #a39697; font-size: 9px; margin-bottom: 12px; }
.knowledge-card-meta .used-knowledge { color: #95424a; }
.knowledge-card strong { display: block; font-size: 13px; font-weight: 550; color: #5e5253; line-height: 1.8; }
.knowledge-card p { font-size: 11px; color: #8e8485; margin: 8px 0 15px; line-height: 1.8; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
.knowledge-card small { color: #a19495; font-size: 10px; }
...
.knowledge-body ul { padding-left: 21px; color: #6c6969; }
.knowledge-body li+li { margin-top: 8px; }
.knowledge-scope { font-size: 14px; color: #7f7071; padding-bottom: 17px; border-bottom: 1px solid #eae8e8; }
.knowledge-used { color: #8d444b; background: #f2f0f0; padding: 10px 12px; border-radius: 8px; font-size: 12px; }
.report-generating { margin: 24px 0; padding: 25px; border: 1px solid #e5e3e3; background: #f8f6f6; border-radius: 12px; }
.report-generating p { margin: 0 0 20px; font-size: 13px; }
.skeleton-line { height: 10px; margin: 12px 0; border-radius: 5px; background: linear-gradient(90deg,#e9e7e7,#f7f5f5,#e9e7e7); background-size: 200% 100%; animation: shimmer 2s linear infinite; }
.skeleton-line:last-child { width: 64%; }
.shimmer-text { color: #7d6e6f; animation: soft-breathe 1.8s ease-in-out infinite; }
.tabs { gap: 26px; }
