...
1. 关键词和向量通道并行执行；首批候选提交后启动学习，后续召回与学习重叠。命中 ID 集合留在数据侧，任务只接收首批展示/种子窗口，正文按标注与查看需要读取。数据库全域数值扫描按原句及用户补充计算相关分数：查询字符 2-gram 在工单索引中的覆盖率与非负向量余弦相似度等权平均。查询向量复用已有 embedding 缓存；词面覆盖复用 gram 索引，未建索引时在数据库搜索字段上计算，只向算子返回数值。
   有界候选池按相关分数降序保留，并与已有召回种子合并；训练、扩充和模型选择都从前往后标注，分数相同时按 ID 稳定排序，不混入随机或 K-means++ 样本，也不在读取正文前重新按 ID 打乱。默认每个发现标注窗口 128 条，按全局排序一起读取训练和留出候选；排名只决定先判断哪条，不能作为业务标签。未决样本不计入有效训练样本，二分类训练需要实际正反例。
...
4. 抽样初判与命中复核共用每个查询条件代次最多 128 次模型请求；失败请求计数，缓存命中不增加，重启与续跑不重置。训练样本用于拟合，互不重叠的选择样本用于选模型和阈值；样本数量随标签分布、未决比例及达标情况变化，不是固定配额。没有选择集达标候选且尚有额度时，按相同顺序补充标注并重新比较；保留历轮最佳模型。仍未达原目标时按选择集 F1 选最佳者，查准率至少 60% 才可预测，否则返回 `model_unknown`。额度用尽且缺少可训练类别或有效选择指标也返回 `model_unknown`；最后一次初判的命中尚未复核时保持未决。
5. 选择集达标或最佳模型达到 60% 查准率门槛后冻结模型和阈值，一遍全范围数值扫描输出预测块。已有正反例与未决标签覆盖预测，缺特征保持未知；不再建立独立随机样本、读取抽验正文或调用大模型。四个模型共用一套训练与选择标签，不进行四次全库扫描。
6. 新结果的质量说明使用 `basis=selection`、`precision`、`recall`、目标和选择样本数量。默认选择集 P/R 目标均为 .95，可配置；不输出总体查准率/召回率下界。未知选择标签不参与二分类指标，数量单独披露。选择指标用于模型与阈值选择，不等于真实业务质量或全库找全。
7. `quality_passed` 表示选择集达标，`quality_fallback` 表示未达原目标而采用最佳模型，两者均已完成全集预测。质量对象用 `acceptance=target/fallback` 区分，Host 同时核验选择集查准率至少 60%。`model_unknown` 显示“不知道”，Host 以未完成结束，页面、报告、下载和重放只返回已有明确确认结果。已有任务的旧抽验指标只保留为历史记录，不重新触发抽验；旧 `validation_size`/`delta` 配置在默认学习路径忽略，显式旧 baseline 配置不变。
...
续跑复用服务自产的有界排序 ID 池与小模型缓存，不复制全库向量。`needs_selection_coverage` 保留原训练/选择划分，继续从未判且未训练的排序池补选择样本，不重做全域发现扫描和模型拟合；新增召回种子按相关分数合入。新选择样本用于经验选模，达标后直接预测，不再独立抽验。查询、授权、来源数值版本或学习配置改变会失效缓存；已有训练标签更正时重训。未决样本按原文及证据要求辨识，相同材料不再次请求模型；补充原文后恢复判断。传输并发和批宽变化不清除训练成果。主 Agent 可直接调用筛选续补选择集，无需先补搜；候选池耗尽仍缺类时返回 `needs_coverage`，要求解决发现缺口。
