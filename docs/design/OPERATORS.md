...
4. 抽样初判与命中复核共用每个查询条件代次最多 128 次模型请求；失败请求计数，缓存命中不增加，重启与续跑不重置。没有选择集合格候选且尚有额度时，按相同顺序补充一次训练窗口并重新比较；保留历轮最佳模型。仍未达原目标时按选择集 F1 选最佳者，查准率至少 60% 才可预测，否则返回 `model_unknown`。额度用尽且缺少可训练类别或有效选择指标也返回 `model_unknown`；最后一次初判的命中尚未复核时保持未决。
