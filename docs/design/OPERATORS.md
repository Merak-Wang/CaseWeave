...
同一次查询规划负责知识路由：检索 Agent 接收固定版本 Wiki 的领域目录、条目标题、适用范围与关键词，返回 `knowledge_routes`（条目 ID、业务选择理由）。规划阶段不自动注入知识正文；抽样判断和复核只加载选中条目的正文，空路由执行零样本判断。页面展示选择和理由。没有路由的历史计划在再次执行时补做规划，不沿用固定前三条的词面搜索结果。

规划上下文区分 `search_fields`（搜索索引名）与 `evidence_fields`（可读来源字段）；`required_fields` 必须来自后者，编造字段在规划阶段反馈修正，不能用不存在的 `body` 阻塞全部样本判断。用户明确要求但来源缺失的原文仍保持未决，不自动换成摘要。
...
4. 没有选择集合格候选时，可按相同相关性顺序补充一次有界训练窗口。随后冻结模型和阈值，一遍全范围数值扫描同时输出预测、计数并保留互斥区域的独立简单随机抽验样本。这个抽验仅用来估计遗漏和误收，不承担发现训练正例的职责；不能把排序靠前的高相关样本代入随机抽验的统计公式。四个模型不需要四次标注或四次全库扫描。
...
未达标返回 `next_action`：缺事实补信息、覆盖不足且仍有排序候选时继续下一窗口，池耗尽再找新表达；预测错误换表示或判别器，区间太宽增加新的独立测量。输入没有变化且无可继续的样本时不重复启动同一失败过滤；新增发现或证据后可重新测量。没有按累计调用/token/耗时宣布完成的门槛，也没有隐式全量 teacher 回退。样本大小、优化器迭代数、HTTP 批宽、线程数均为运行配置。

续跑复用服务自产的有界排序 ID 池与小模型缓存，不复制全库向量。`needs_selection_coverage` 保留原训练/选择划分，继续从未判且未训练的排序池补选择样本，不重做全域发现扫描和模型拟合；新增召回种子按相关分数合入。新选择样本只供经验选模，最终验收仍使用冻结后的独立总体抽验。查询、授权、来源数值版本或学习配置改变会失效缓存；已有训练标签更正时重训。未决样本按原文及证据要求辨识，相同材料不再次请求模型；补充原文后恢复判断。传输并发和批宽变化不清除训练成果。主 Agent 可直接调用筛选续补选择集，无需先补搜；候选池耗尽仍缺类时返回 `needs_coverage`，要求解决发现缺口。
