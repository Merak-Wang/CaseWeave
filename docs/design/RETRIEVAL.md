...
默认 `auto/learned` 路径由 Host 并行启动数据库关键词集合生成和 Milvus 向量召回。SQL 通过 `INSERT … SELECT` 将全部字面命中写入持久候选集合，Milvus 查询及合并独立推进；首批候选可见后可供 Agent 评估，后续召回结果继续汇入同一集合。只有全部计划启用的通道完成并提供完整召回范围后，才启动 Python 全集过滤与学习。Provider 的 `recallScope` 指向完成的搜索运行；特征种子、分页扫描、按 ID 取块及预测都限制在该运行的关键词命中与语义命中去重并集中，缺少完整 scope 时保持未完成，不回退到全授权语料。过滤只读取按相关性排序的有界标注/选择样本原文，训练与选择完成后以数值特征块预测该并集剩余范围，不逐页读取全库正文。预测通过选择集质量准入后写入持久学习结果集，确认结果视图、报告和下载按该集合交付；既有逐条 teacher 判断保留实际证据，纯模型推断成员以 proxy 标识，不代表另有逐条 LLM 强判断或证据引用。
...
数据库快照保存版本、授权绑定和字段目录，不持有整代工单正文。`ra_ticket_access` 保存可索引的访问属性和 L0/L1 投影，`ra_search_candidate` 保存两路并集及可索引排序，`ra_provider_candidate` 保存已发出的稳定引用。搜索页的 `recallScope` 是 `ra_search_run.id`；收到 scope 的数值特征查询会关联该运行的 `ra_search_candidate`，并把范围身份纳入 `feature_id`，因此分页扫描、显式 ID 读取和模型结果缓存都不能越出这次召回集合。Local Provider 以快照内完整排名缓存执行同一范围约束。初始化为旧代补齐缺失投影；查询、恢复和进度只读取当前窗口，原文与向量片段身份核对按工单 ID 读取。`ra_search_hit` 继续保留通道审计。
...
验收重点：跨字段 AND、嵌套 OR/NOT、空值和日期、字面原因区分、关键词全集与分页一致、SQL/文件匹配一致、Milvus 真实调用、陈旧索引与删除、单路失败、千级候选内存/首屏、召回并集跨首屏分页的 scope 一致性、范围外强阳性特征不能进入预测集合、空召回保持为空、取消后的模型服务占用。加速效果必须同时报告命中集合差异与延迟，不用“变快了”掩盖漏检。

规划后的学习式检索由 Provider 并行执行关键词与向量通道。关键词 OR 在 SQL 内一次生成命中 ID 集合，同一词表共享一次字段扫描；不按 500 条重跑谓词，也不要求 Host 按 100 条把全部正文搬入任务状态。首批候选可先展示；关键词全集枚举与计划启用的语义召回都完成后，Host 才启动学习，并把完整召回运行的 `recallScope` 传给 Provider 与 Python 算子。抽样、训练和数值预测都在两路去重并集内，首屏页宽不改变总体范围，也不把全库授权范围当作总体。显式案例/direct/baseline 保留按需分页路径。
