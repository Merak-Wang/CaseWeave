...
查询理解先生成查询计划，再由 MySQL 与 Milvus 并行执行关键词和向量召回；结果进入 MySQL 任务状态，再由 DSH 主 Agent 和领域专家复核。浏览器工作台订阅持久事件，报告与下载绑定同一确认结果版本。Wiki 使用固定版本读取和异步学习发布；检索质量及知识收益需在指定数据与模型下评测。
...
flowchart TD
  ...
  Q --> K[MySQL 关键词 OR 全部命中]
  Q --> V2[语义改写与 Milvus 召回]
  V2 --> C[关键词命中与语义召回按工单身份去重并集]
  K --> C
  C --> O[Python filter：候选并集学习与预测，每代最多128次独立判断，重试不扣额度]
  F[召回并集 ID 对齐的连续向量/CSR] --> O
  O -->|DSH 强判断样本与质量| S
  O --> M[召回并集内的数值预测集合]
  M -->|整体质量准入后作为学习结果分页| S
  K --> S[权威任务状态与证据集合]
...
查询规划完成后，关键词与语义向量召回并行执行，任一路产出即可更新过程视图，另一分支状态仍清楚可见。首批候选允许先展示并由 Agent 评估；数值学习必须等关键词枚举与计划启用的语义召回都完成，再对两路去重后的完整并集抽样和预测，不能把首屏窗口当作学习总体。Provider 搜索页提供已完成运行的 `recallScope`，特征读取通过该句柄关联候选集合；`features.seeds`、扫描、取块与预测都继承同一范围。缺少完整范围时学习保持未完成，不回退到全部授权工单。完整关键词枚举继续进入同一候选库。不要用“等最慢分支和所有页面结束”实现首屏，也不要因为开始 Agent 决策而截断关键词枚举。

语义能力只保留 filter、extract、agg。默认学习范围为关键词 OR 全部命中与语义召回结果去重后的候选并集，不扩展为全部授权工单；并集内不逐条 LLM 强判，数值预测与真实 teacher 判断分开表达。数据库 Provider 将 `recallScope` 绑定到已完成搜索运行，并把它纳入特征身份，防止跨范围复用训练缓存；Local Provider 从该快照的完整排名结果限定特征。范围句柄和质量对象随输入代次失效。满足选择集质量准入后，预测成员写入持久学习结果集，确认视图、报告与下载按该集合分页交付；已有逐条 teacher 判断保留其实际证据，模型推断成员标为 proxy，不伪装成逐条 LLM 判断或证据引用。确认数量和分页使用持久集合，已水合样本只作为证据窗口。具体调用与统计假设见 [算子设计](design/OPERATORS.md)。
