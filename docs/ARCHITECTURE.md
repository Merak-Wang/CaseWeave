...
原句向量与查询规划实际并行；规划完成后执行关键词和语义改写，任一路产出即可更新过程视图，另一分支状态仍清楚可见。首批候选允许先展示并由 Agent 评估；数值学习必须等关键词枚举与计划启用的语义召回都完成，再对两路去重后的完整并集抽样和预测，不能把首屏窗口当作学习总体。Provider 搜索页提供已完成运行的 `recallScope`，特征读取通过该句柄关联候选集合；`features.seeds`、扫描、取块与预测都继承同一范围。缺少完整范围时学习保持未完成，不回退到全部授权工单。完整关键词枚举继续进入同一候选库。不要用“等最慢分支和所有页面结束”实现首屏，也不要因为开始 Agent 决策而截断关键词枚举。
